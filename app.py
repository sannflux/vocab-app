...
def highlight_vocab(text, vocab_raw):
    # Note: For cloze deletions, we don't necessarily need HTML bolding here, 
    # but we keep it for reference if needed. The cloze syntax handles the hiding.
...
def process_anki_data(df, batch_size=5):
    ...
    for card_data in all_card_data:
        ...
        # Create Cloze deletion: "I found it by {{c1::serendipity}}!"
        # We need a case-insensitive replacement for the cloze logic
        if phrase and vocab_raw:
            pattern = re.compile(re.escape(vocab_raw), re.IGNORECASE)
            # Replace occurrences with the cloze syntax, preserving the case found in the sentence
            phrase_cloze = pattern.sub(lambda m: f"{{{{c1::{m.group(0)}}}}}", phrase)
        else:
            phrase_cloze = f"{{{{c1::{vocab_cap}}}}}"
        ...
        # Text field contains the Sentence with Cloze deletion
        text_field = phrase_cloze
...
"""
    ...
    # 2. Define Deck (Dynamic Name)
    # Generate a fairly unique ID based on the deck name hash
...
with tab3:
    ...
    if df.empty:
        ...
        if st.button("🚀 Generate Deck", type="primary", use_container_width=True):
            ...
            st.dataframe(pd.DataFrame(raw_notes)[['VocabRaw', 'Definition']].head(3), use_container_width=True)

st.caption("✅ Cyberpunk Theme + Custom Deck Name + Audio")
