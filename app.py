...
# ========================== GENERATE NOTES (now accepts batch_size) ==========================
...
# ========================== ANKI TAB WITH BATCH SLIDER ==========================
with tab3:
    st.subheader("📇 Generate Anki (AI) — 100% identical to Colab")
    if df.empty:
        st.info("Add words first!")
    else:
        st.write(f"Ready to generate **{len(df)}** cards")
        
        # === NEW BATCH SIZE SLIDER ===
        batch_size = st.slider(
            "🔢 Batch size (higher = faster, lower = safer on free tier)",
            min_value=1,
            max_value=10,
            value=5,
            step=1,
            help="Recommended: 5 for free Gemini. Increase only if you have a paid key."
        )
        
        if st.button("🚀 Generate Anki Cards with Gemini AI", type="primary", use_container_width=True):
            with st.spinner(f"🧠 Generating in batches of {batch_size}..."):
                ...
                st.success(f"🎉 {len(anki_df)} cards ready (batched in groups of {batch_size})!")
...
st.caption("✅ Batch slider added • Synonyms capitalized • Sentence casing fixed • API key changer")        for attempt in range(max_retries):
            response = run_with_timeout(model.generate_content, args=(prompt,), timeout=40)
            if response is None:
...
st.caption("✅ 100% identical to Colab + API key changer + better sentence capitalization")            if st.button("🔊 Speak Vocab", key="q_v"): speak_word(quick_word, speech_lang)
        with c2: 
            if quick_phrase.strip() and st.button("🔊 Speak Phrase", key="q_p"): speak_word(quick_phrase, speech_lang)
