...
def save_to_github(dataframe):
    """B4: Updates session_state immediately; GitHub write is the only I/O."""
    t0        = perf_counter()
    dataframe = (
    ...
    load_data.clear()   # invalidate read-cache so next cold-start gets fresh data
...
def save_single_word_callback():
    ...
    if v:
        p = p_raw.strip()
        if p and p not in ("1", "*") and not p.startswith("*"):
            if p.endswith(","): p = p[:-1] + "."
    ...
    if add_mode == "Single":
        ...
        if st.session_state.bulk_preview_df is not None:
            ...
            if st.button("💾 Confirm & Process Bulk", type="primary"):
                ...
                # A3: Removed time.sleep(0.5) dead-wait
                st.rerun()
