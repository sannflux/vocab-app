...
def get_last_update_str():
    try:
        ...
        if commits:
            utc_dt = commits[0].commit.committer.date
...
if "last_update" not in st.session_state:
    st.session_state.last_update = get_last_update_str()

df = st.session_state.df

# ========================== MAIN UI ==========================
st.caption(f"🌐 **Last updated:** {st.session_state.last_update} (WIB)")
...
with st.sidebar:
    ...
    if st.button("🔄 Refresh from Cloud", use_container_width=True, type="secondary"):
        st.session_state.df = load_df()
        st.session_state.last_update = get_last_update_str()
        st.toast("✅ Synced with GitHub!", icon="🔄")
        st.rerun()
...
with tab1:
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        submitted = st.form_submit_button("💾 Save to Cloud" if not exists else "🔄 Update Phrase", use_container_width=True)
    ...
    if submitted and v:
        ...
        if exists:
            ...
            st.session_state.last_update = (datetime.now(timezone.utc) + timedelta(hours=7)).strftime("%d %b %Y, %H:%M WIB")
            st.toast("✅ Phrase updated!", icon="💾")
        else:
            updated = pd.concat([df, pd.DataFrame([{"vocab": v, "phrase": p}])], ignore_index=True)
            save_to_github(updated)
            st.session_state.df = updated
            st.session_state.last_update = (datetime.now(timezone.utc) + timedelta(hours=7)).strftime("%d %b %Y, %H:%M WIB")
            st.toast("✅ New word added!", icon="💾")
        st.rerun()
...
with tab2:
    if df.empty:
        ...
        edited = st.data_editor(
            ...
            hide_index=True,
            column_config={"vocab": st.column_config.TextColumn("Vocab", disabled=True)}
        )
        
        if st.button("💾 Save Changes", type="primary", use_container_width=True):
            if search:
                # Proper merge: keep non-matching rows + updated/deleted matching rows
                non_matching = df[~df['vocab'].str.contains(search, case=False)]
                to_save = pd.concat([non_matching, edited], ignore_index=True)
            else:
                to_save = edited
            
            sorted_df = to_save.sort_values(by="vocab", ignore_index=True)
            save_to_github(sorted_df)
            st.session_state.df = sorted_df
            st.session_state.last_update = (datetime.now(timezone.utc) + timedelta(hours=7)).strftime("%d %b %Y, %H:%M WIB")
            st.toast("✅ Changes saved to cloud!", icon="💾")
            st.rerun()
        
        csv = edited.to_csv(index=False).encode()
        st.download_button("📥 Download CSV", csv, "vocab.csv", "text/csv", use_container_width=True)
...
st.caption("✅ Fixed search+delete • Refresh button • Auto-rerun after save • Stats in sidebar")
