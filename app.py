...
BATCH INPUT: {json.dumps(batch_dicts, ensure_ascii=False)}"""
        ...
        for attempt in range(4):
            # Using ThreadPoolExecutor for clean timeout handling
...
def process_anki_data(df, batch_size=5):
    # Safely drop NA vocabs before to_list to avoid breaking the generator
...
def save_to_github(dataframe):
    ...
    # Clear cache upon successful write
    load_data.clear() 
    return True

# Ensure we use a copy to prevent "Mutated cached value" warnings from Streamlit
df = load_data().copy()
