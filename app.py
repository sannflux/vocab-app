...
def generate_anki_card_data_batched(vocab_phrase_list, batch_size=6):
    ...
    with st.status("🤖 Processing AI Batches (RPM Throttled)...", expanded=True) as status_log:
        ...
        for idx, batch in enumerate(batches):
            ...
            # RPM Enforcer (5 requests/minute = 12.5s delay)
            if idx > 0:
                for remaining in range(13, 0, -1):
                    progress_bar.progress(idx / len(batches), text=f"⏳ RPM Safeguard Throttle... ({remaining}s)")
                    time.sleep(1)
...
6. MANDATORY: 'translation' must contain ONLY the {TARGET_LANG} translation of the 'vocab' word. Do NOT translate the whole phrase.
