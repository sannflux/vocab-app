...
OUTPUT FORMAT: [{{"vocab": "...", "phrase": "...", "translation": "{USER_NATIVE_LANGUAGE} meaning", "part_of_speech": "...", "pronunciation_ipa": "/.../", "definition_english": "...", "example_sentences": ["..."], "synonyms_antonyms": {{"synonyms": [], "antonyms": []}}, "etymology": "Plain text only."}}]
...
def process_anki_data(df, batch_size=6):
    vocab_phrase_list = df.dropna(subset=['vocab'])[['vocab', 'phrase']].values.tolist()
...
def create_anki_package(notes_data, deck_name, generate_audio=True):
    ...
    # --- CYBERPUNK CSS (100% original from your code) ---
    cyberpunk_css = """
...
"""
    ...
    # 2. Dynamically Generate Model ID
    ...
    # 5. Create Temp Directory for MP3s
...
with st.sidebar:
    ...
    if wotd_vocab:
        ...
        with c2: 
            if wotd_phrase and wotd_phrase.strip() and st.button("🔊 Phrase", key="wotd_p"): speak_word(wotd_phrase, speech_lang)
...
with tab3:
    ...
    if df.empty:
        ...
        if st.button("🚀 Generate Deck", type="primary", use_container_width=True):
            if len(df) > 400:
                st.warning("⚠️ Large deck detected — this may take 1–3 minutes")
