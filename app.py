...
import os
import tempfile
from zoneinfo import ZoneInfo   # ← Added for correct Jakarta time
...
def load_data():
    try:
        ...
        # NEW: Get REAL last updated time from GitHub commit
        try:
...
NEVER use markdown... (same as before)
...
def process_anki_data(df, batch_size=5):
    ...
    for card_data in all_card_data:
        # ... (full processing logic unchanged - highlight, clean, Text field, etc.)
...
# ========================== GENANKI & AUDIO LOGIC (full unchanged) ==========================
...
.vellum-focus-container { background: #0d0d0d; padding: 30px 20px; margin: 0 auto 40px; max-width: 95%; border-radius: 4px; border: 2px solid #00ff41; box-shadow: 0 0 5px #00ff41, 0 0 15px rgba(0, 255, 65, 0.4), 0 4px 8px rgba(0, 0, 0, 0.5); text-align: center; position: relative; overflow: hidden; }
.nightMode .vellum-focus-container { border: 2px solid #00aaff; box-shadow: 0 0 5px #00aaff, 0 0 15px rgba(0, 170, 255, 0.4), 0 4px 8px rgba(0, 0, 0, 0.6); }
.prompt-text { font-family: 'Electrolize', 'Arial Narrow', sans-serif; font-size: clamp(1.5em, 5vw, 2.0em); font-weight: 900; color: #ffffff; text-shadow: 1px 1px 0 #ff00ff, -1px -1px 0 #00ffff; font-style: normal; }
.nightMode .prompt-text { color: #f0f8ff; text-shadow: 1px 1px 0 #ff00ff, -1px -1px 0 #00ffff; }
.vellum-focus-container.front .cloze { color: #111111; background-color: #00ff41; border-radius: 2px; padding: 2px 4px; line-height: 1; text-decoration: none; font-style: normal; }
.nightMode .vellum-focus-container.front .cloze { background-color: #00aaff; color: #0d0d0d; }
.vellum-focus-container.back .prompt-text { color: #e0e0e0; }
.vellum-focus-container.back .cloze { color: #ff00ff; font-weight: 900; background: none; padding: 0 3px; text-decoration: none; border-bottom: 3px double #00ffff; text-shadow: 0 0 5px #ff00ff; font-style: normal; }
.nightMode .vellum-focus-container.back .cloze { color: #00ffff; border-bottom: 3px double #ff00ff; text-shadow: 0 0 5px #00ffff; }
.vellum-detail-container { padding: 10px 0; }
.vellum-section { margin: 15px 0; padding: 10px 0; border-bottom: 1px dashed #00ff41; padding-left: 5px; padding-right: 5px; }
.nightMode .vellum-section { border-bottom: 1px dashed #00aaff; }
.section-header { font-size: 1.1em; font-weight: 600; margin-bottom: 8px; color: #00ffff; display: flex; align-items: center; gap: 8px; border-left: 3px solid; padding-left: 10px; text-transform: uppercase; letter-spacing: 1px; }
.nightMode .section-header { color: #ff00ff; }
.content { font-size: 0.95em; color: #aaffaa; padding: 0 0 0 13px; }
...
.pronunciation .content { font-family: 'Consolas', monospace; font-size: 1.05em; font-weight: 500; color: #ffff00; }
.examples .content { color: #77ff77; font-style: italic; font-size: 0.9em; }
@media (max-width: 480px) { .card { font-size: 16px; padding: 15px 10px; } .vellum-focus-container { padding: 20px 10px; max-width: 100%; } }
"""

    front_html = """<div class="vellum-focus-container front"><div class="prompt-text">{{cloze:Text}}</div></div>"""
    back_html = """<div class="vellum-focus-container back"><div class="prompt-text solved-text">{{cloze:Text}}</div></div><div class="vellum-detail-container">{{#Definition}}<div class="vellum-section definition"><div class="section-header">📜 DEFINITION</div><div class="content">{{Definition}}</div></div>{{/Definition}}{{#Pronunciation}}<div class="vellum-section pronunciation"><div class="section-header">🗣️ PRONUNCIATION</div><div class="content">{{Pronunciation}}</div></div>{{/Pronunciation}}{{#Examples}}<div class="vellum-section examples"><div class="section-header">🖋️ EXAMPLES</div><div class="content">{{Examples}}</div></div>{{/Examples}}{{#Synonyms}}<div class="vellum-section synonyms"><div class="section-header">➕ SYNONYMS</div><div class="content">{{Synonyms}}</div></div>{{/Synonyms}}{{#Antonyms}}<div class="vellum-section antonyms"><div class="section-header">➖ ANTONYMS</div><div class="content">{{Antonyms}}</div></div>{{/Antonyms}}{{#Etymology}}<div class="vellum-section etymology"><div class="section-header">🏛️ ETYMOLOGY</div><div class="content">{{Etymology}}</div></div>{{/Etymology}}<div style='display:none'>{{Audio}}</div></div>{{Audio}}"""

    model_id = 1607392325
    my_model = genanki.Model(model_id, 'Cyberpunk Vocab Model', fields=[{'name': 'Text'},{'name': 'Pronunciation'},{'name': 'Definition'},{'name': 'Examples'},{'name': 'Synonyms'},{'name': 'Antonyms'},{'name': 'Etymology'},{'name': 'Audio'},], templates=[{'name': 'Cyberpunk Card', 'qfmt': front_html, 'afmt': back_html,}], css=cyberpunk_css, model_type=genanki.Model.CLOZE)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, note_data in enumerate(notes_data):
            ...
            my_note = genanki.Note(model=my_model, fields=[note_data['Text'], note_data['Pronunciation'], note_data['Definition'], note_data['Examples'], note_data['Synonyms'], note_data['Antonyms'], note_data['Etymology'], audio_field])
...
with st.sidebar:
    ...
    # FIXED TIMESTAMP - real GitHub time in your Jakarta timezone
