...
# ========================== LOAD / SAVE / LAST UPDATE (WIB) ==========================
def load_df():
    """Load vocabulary from GitHub (used only on first run or manual refresh)."""
    try:
...
def get_last_update_str():
    """Get last commit time converted to WIB (Indonesia Western Time, UTC+7)."""
    try:
...
# Show last update in WIB right under title
    ...
    if submitted and v:
        ...
        if exists:
            ...
            if save_to_github(df):
                ...
                st.session_state.last_update = wib_now.strftime("%d %b %Y, %H:%M WIB")
                st.toast("✅ Phrase updated in cloud!", icon="💾")
        else:
            ...
            if save_to_github(updated):
                ...
                wib_now = now_utc + timedelta(hours=7)
                st.session_state.last_update = wib_now.strftime("%d %b %Y, %H:%M WIB")
                st.toast("✅ New word saved to cloud!", icon="💾")
    ...
    if df.empty:
        ...
        with col1:
            if st.button("💾 Save Changes", type="primary", use_container_width=True):
                to_save = edited if not search else df.copy()
                if search:
                    # Merge edits back to full list (fixes original bug)
                    for _, row in edited.iterrows():
                        v = row['vocab']
                        to_save.loc[to_save['vocab'] == v, 'phrase'] = row['phrase']
                
                sorted_df = to_save.sort_values(by="vocab", ignore_index=True)
                if save_to_github(sorted_df):
                    st.session_state.df = sorted_df
                    now_utc = datetime.now(timezone.utc)
                    wib_now = now_utc + timedelta(hours=7)
                    st.session_state.last_update = wib_now.strftime("%d %b %Y, %H:%M WIB")
                    st.toast("✅ Full list saved to cloud!", icon="💾")
        with col2:
...
st.caption("✅ Restored original format: Phrase + Vocab: {{c1::Translation}} | Last update shown in WIB | Saving now uses session cache (instant UI)")
